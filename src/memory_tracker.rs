...
impl MemoryPressure {
    ...
    pub fn from_usage(current_mb: f64, peak_mb: f64) -> Self {
        let usage_ratio = if peak_mb > 0.0 {
            current_mb / peak_mb
        } else {
            0.0
        };
...
impl MemoryTracker {
    ...
    fn get_recommendations(&self) -> Vec<String> {
        ...
        if usage.avg_allocation_size > 1024.0 * 1024.0 {
            // > 1MB average
            recommendations.push(
                "Large average allocation size detected - consider buffer pooling".to_string(),
            );
        }
