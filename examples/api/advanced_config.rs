...
use turbo_cdn::{TurboCdn, DownloadOptions, Result};
...
async fn main() -> Result<()> {
    ...
    println!("  🧩 Max concurrent chunks: {}", download_options.max_concurrent_chunks);
    println!("  📦 Chunk size: {} MB", download_options.chunk_size / (1024 * 1024));
    println!("  🔄 Resume enabled: {}", download_options.enable_resume);
    println!("  ⏱️  Timeout: {}s", download_options.timeout_override.unwrap().as_secs());
    println!("  🛡️ Integrity verification: {}", download_options.verify_integrity);
    ...
    let performance_options = DownloadOptions {
        ...
        progress_callback: Some(Box::new(|progress| {
            if progress.percentage as u32 % 10 == 0 {
                println!("🚀 High-speed download: {:.0}% complete", progress.percentage);
            }
        })),
    };
    
    println!("Performance-optimized settings:");
    println!("  ⚡ Max concurrent chunks: {}", performance_options.max_concurrent_chunks);
    println!("  📦 Chunk size: {} MB", performance_options.chunk_size / (1024 * 1024));
    println!("  🛡️ Integrity verification: {}", performance_options.verify_integrity);
    ...
    let conservative_options = DownloadOptions {
        ...
        custom_headers: Some({
            ...
            headers.insert("User-Agent".to_string(), "TurboCDN/1.0 (Conservative Mode)".to_string());
            headers
        }),
        ...
        expected_size: None,
        progress_callback: Some(Box::new(|progress| {
            println!("🐌 Conservative download: {:.1}% - {:.2} MB/s", 
                progress.percentage, progress.speed / 1_000_000.0);
        })),
    };
    
    println!("Conservative settings:");
    println!("  🐌 Max concurrent chunks: {}", conservative_options.max_concurrent_chunks);
    println!("  📦 Chunk size: {} KB", conservative_options.chunk_size / 1024);
    println!("  🛡️ Integrity verification: {}", conservative_options.verify_integrity);
    ...
    println!("  ⏱️  Timeout: {}s", env_options.timeout_override.unwrap_or(Duration::from_secs(30)).as_secs());
    ...
    println!("  📊 Detected optimal chunks: {}", network_config.max_concurrent_chunks);
    println!("  📦 Optimal chunk size: {} KB", network_config.chunk_size / 1024);
...
fn get_file_type_configurations() -> HashMap<String, DownloadOptions> {
    ...
    configs.insert("Large Binary".to_string(), DownloadOptions {
    ...
    configs.insert("Source Code".to_string(), DownloadOptions {
    ...
    configs.insert("Small Tools".to_string(), DownloadOptions {
